...
public abstract class BuildSda extends DefaultTask {
    @Input
    ...
    @InputFile
    public abstract RegularFileProperty getSrcFile();
    ...
    public void buildSDA() throws IOException {
        String fileName = getSrcFile().get().getAsFile().getName();
        Objects.requireNonNull(fileName);
        boolean isRAR = fileName.endsWith(".rar");
        boolean isEJB = fileName.endsWith(".ejb.jar");
        boolean isWAR = fileName.endsWith(".war");
        boolean isLib = !isEJB && fileName.endsWith(".jar");
...
//        props.loadFromXML(new FileInputStream(getPropertyXml().get().getAsFile()));
        ...
        File warFile = getSrcFile().getAsFile().get();
